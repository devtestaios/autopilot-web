...
export function useGlobalSearch(searchData?: SearchData) {
  ...
  // Use ref to store the latest searchable content without causing re-renders
  const searchableContentRef = useRef<SearchResult[]>([]);
  ...
  const allSearchableContent: SearchResult[] = useMemo(() => {
    ...
    return content;
  }, [searchData?.campaigns?.length, searchData?.leads?.length, searchData?.templates?.length]);
  ...
  }, [debouncedSearchTerm]); // Remove allSearchableContent from dependencies
