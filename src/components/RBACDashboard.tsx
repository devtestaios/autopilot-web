...
export default function RBACDashboard() {
  ...
  const getPermissionIcon = (permissionId: string) => {
    ...
    if (!permission) return <Key className="w-4 h-4" />;
    
    switch (permission.category) {
      case 'campaign_management': return <BarChart3 className="w-4 h-4" />;
  ...
  const calculateOverviewStats = () => {
    const totalPermissions = PERMISSION_REGISTRY.length;
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {Object.entries(
                  PERMISSION_REGISTRY.reduce((acc, permission) => {
